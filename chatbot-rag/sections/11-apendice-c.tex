...
\addcontentsline{toc}{section}{APÊNDICE \Alph{section} -- PROMPT CONFIGURADO NO MODELO RAG DO OPEN WEBUI}
\begin{center}
\textbf{APÊNDICE \Alph{section} -- PROMPT CONFIGURADO NO MODELO RAG DO OPEN WEBUI}
\end{center}
\label{sec:apendice-prompt-rag}

Este apêndice registra o prompt configurado no modelo RAG do Open WebUI\footnote{\url{https://github.com/fabriciosantana/mcdia/blob/main/05-iag/4-project/eval/prompts/rag_prompt.md}}. Trata-se do artefato operacional associado ao uso interativo do sistema na própria plataforma, e não do \textit{system prompt} externo empregado pelo script na bateria automatizada. Sua formulação explicita as restrições centrais adotadas no fluxo RAG, sobretudo quanto à fidelidade ao contexto recuperado, controle de escopo, reconhecimento de limites do contexto, separação entre contexto e conhecimento externo, distribuição de citações ao longo da resposta e exigência de uma seção final de referências.

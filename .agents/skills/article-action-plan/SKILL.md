...
- `[~]` means in progress.
- `[x]` means completed.
- Do not mark an action complete merely because it was discussed.
- If the work reveals a dependency or blocker, record that explicitly under the action.

## Prioritization

- Prefer actions that unlock later work, define the editorial target, strengthen contribution claims, or improve evidence quality.
- For article-to-journal work, establish the target journal before making large structural changes.
- Avoid reopening already settled delivery versions unless the user asks for a new publication-oriented revision.
- Separate quick editorial wins from actions that require new experiments, new data collection, human annotation, or external publication-policy checks.
- If a task requires current journal policies, editorial status, rankings, or publication venues, verify against current official sources before acting.

## Action Completion Standard
